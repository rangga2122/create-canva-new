...
class LeonardoGUI(ctk.CTk):
    ...
    def on_open_browser(self):
        ...
        self.log("Silakan login ke: Firefox Relay, Gmail, Canva, dll.", "info")
    ...
    async def _open_browser_async(self):
        ...
        async with async_playwright() as p:
            ...
            # Open useful pages
            await page.goto("https://relay.firefox.com/", wait_until="domcontentloaded", timeout=30000)
            
            self.after(0, lambda: self.log("Login ke akun yang diperlukan, lalu tutup browser.", "info"))
            
            # Wait until browser is closed by user
            try:
                await browser.wait_for_event("close", timeout=0)
            except Exception:
                # Browser closed or timeout=0 not supported, poll instead
                while True:
                    try:
                        pages = browser.pages
                        if not pages:
    ...
    def _on_browser_closed(self):
        ...
        self.browser_running = False
        self.browser_btn.configure(state="normal", text="\U0001F310  BUKA BROWSER")
        self.start_btn.configure(state="normal")
        self.log("Browser ditutup. Siap menjalankan bot!", "ok")
